...
const FingerprintDashboard = () => {
  ...
  const [showAfterEnrollPrompt, setShowAfterEnrollPrompt] = useState(false);
  ...
  const formatSerialLogLine = (line) => {
    const raw = String(line || '').trim();
    if (!raw || /^\.+$/.test(raw)) return null;

    if (raw.includes('Port open')) return 'USB connected to Pico at 115200 baud.';
    if (raw.includes('Sent ENROLL')) return 'Enrollment command sent to the sensor.';
    if (raw.includes('Follow prompts on the sensor')) return 'Follow the sensor prompts to scan your finger twice.';
    if (raw.includes('Image taken')) return 'Fingerprint image captured.';
    if (raw.includes('Image converted')) return 'Image processed and converted.';
    if (raw.includes('Remove finger')) return 'Remove your finger from the sensor.';
    if (raw.includes('Place same finger again')) return 'Place the same finger again for the second scan.';
    if (raw.includes('Creating model for')) return 'Building a fingerprint model for this scan.';
    if (raw.includes('Fingerprints did not match')) return 'Fingerprints did not match. Try the same finger again.';
    if (raw.includes('Prints matched')) return 'Fingerprint match confirmed.';
    if (raw.startsWith('ID ')) return `Assigned fingerprint slot ${raw.slice(3)}.`;
    if (raw.includes('Ready to enroll a fingerprint')) return 'Sensor ready for a new enrollment.';
    if (raw.includes('Please type in the ID #')) return 'The sensor is asking for the template ID number.';
    if (raw.includes('Waiting for valid finger')) return 'Waiting for a valid finger scan.';
  ...
  const resetEnrollForm = () => {
    ...
    setEnrollError(null);
    setSerialLog([]);
    setShowAfterEnrollPrompt(false);
  };
  ...
  const handleEnrollSubmit = async (e) => {
    ...
    try {
      ...
      await load();
      setEnrollSuccess(`${v.name} linked to template slot ${v.slot}. The new enrollment is now visible in Enrolled fingerprints.`);
      resetEnrollForm();
      setShowAfterEnrollPrompt(false);
      setEnrollOpen(false);
    } catch (err) {
  ...
  const handleUsbCapture = async () => {
    ...
    if (captureOk) {
      ...
      msg += ' The new fingerprint is now visible in Enrolled fingerprints.';
            ...
            <div className="p-6 space-y-4">
              ...
              {enrollTab === 'capture' && (
                <div className="space-y-3 pt-1">
                  ...
                  {serialLog.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">Sensor / Device Log</p>
                      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4 max-h-96 overflow-y-auto" ref={(el) => { if (el) el.scrollTop = el.scrollHeight; }}>
                        <div className="text-sm font-mono text-gray-300 whitespace-pre-wrap break-words leading-6 space-y-1">
                          {serialLog.map((line, idx) => {
                            const formatted = formatSerialLogLine(line);
                            if (!formatted) return null;

                            let color = 'text-gray-400';
                            let prefix = '• ';

                            if (formatted.includes('success') || formatted.includes('confirmed') || formatted.includes('saved')) {
                              color = 'text-green-400 font-semibold';
                              prefix = '✓ ';
                            } else if (formatted.includes('mismatch') || formatted.includes('try again')) {
                              color = 'text-yellow-400 font-semibold';
                              prefix = '⚠ ';
                            } else if (formatted.toLowerCase().includes('error')) {
                              color = 'text-red-400 font-semibold';
                              prefix = '✗ ';
                            } else if (formatted.includes('Fingerprint image captured')) {
                              color = 'text-blue-400';
                              prefix = '📸 ';
                            } else if (formatted.includes('USB connected') || formatted.includes('Enrollment command sent')) {
                              color = 'text-cyan-400 font-semibold';
                              prefix = '🔌 ';
                            } else if (formatted.includes('Waiting for')) {
                              color = 'text-blue-300';
                              prefix = '⏳ ';
                            } else if (formatted.includes('Place') || formatted.includes('Remove')) {
                              color = 'text-purple-400 font-semibold';
                              prefix = '👆 ';
                            } else if (formatted.includes('Building a fingerprint model')) {
                              color = 'text-blue-400';
                              prefix = '⚙️ ';
                            }
                  ...
                  {!showAfterEnrollPrompt ? (
                    ...
                    <div className="rounded-2xl bg-blue-50 border border-blue-100 p-4 space-y-4">
                      <p className="text-sm font-medium text-blue-900">
                        Enrollment complete. Do you want to enroll another fingerprint?
                      </p>
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <button
                          type="button"
                          onClick={handleAddAnother}
                          className="flex-1 px-4 py-2.5 rounded-lg bg-primary text-white font-medium hover:opacity-90"
                        >
                          Yes, add another
                        </button>
                        <button
              ...
              {enrollTab === 'database' && !showAfterEnrollPrompt && (
              ...
              {showAfterEnrollPrompt && enrollTab === 'database' && (
                <div className="rounded-2xl bg-blue-50 border border-blue-100 p-4 space-y-4">
                  <p className="text-sm font-medium text-blue-900">
                    Enrollment complete. Do you want to enroll another fingerprint?
                  </p>
